...
const Indicator = GObject.registerClass(
    class Indicator extends PanelMenu.Button {
            ...
            fmh.sync(profile, (profile, status, message) => {
              ...
              if (status !== fmh.ProfileStatus.BUSSY) {
                fmh.initFilemonitor(profile,
                    function(profile, status, message) {
                      that._onProfileStatusChanged(profile, status, message);
                    });
              }
            });
      ...
      _buildMainMenu(profiles) {
        ...
        // clean menu
        this.menu._getMenuItems().forEach(function(i) {
          i.destroy();
        });
      ...
      // clean submenu
        ...
        menuItem.menu._getMenuItems().forEach(function(i) {
          i.destroy();
        });
          ...
          case 'Disengage':
            ...
            fmh.disengage(menuItem.profile,
                (profile, status, message) => {
                  this._onProfileStatusChanged(profile, status, message);
                });
            break;
          case 'Sync':
      ...
      _onProfileStatusChanged(profile, status, message) {
        try {
          ...
          if (message) {
            this._addLog(profile, _(message));
          }
